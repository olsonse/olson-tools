...
extern "C" {
    /* static */ double gauss_deviate( const double & sigma);
                 double gauss_deviate__(const double sigma[]);
...
/** for g77.  other fortran compilers should call this function using the
 * FFUNC_ macro in fmacros.h
 */
double gauss_deviate__(const double sigma[]) {
    return gauss_deviate(*sigma);
}
