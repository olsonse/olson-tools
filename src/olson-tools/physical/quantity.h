...
#define PHYSICAL_QUANTITY_INITIALIZER_1(c)     Quantity(c)
#define PHYSICAL_QUANTITY_INITIALIZER_2(c,u)   Quantity(c,u)
#define PHYSICAL_QUANTITY_INITIALIZER_3(c,u,n) Quantity(c,u,n)
...
namespace physical {
    ...
    namespace registry {
        ...
        template<class sub>
        struct recorder {
            static  std::map<std::string, sub*> list;

            recorder(const name & n = "") {
                if (n.val != "")
                    record(n);
            }

            sub & record(const name & n) {
                list[n.val] = (sub*)this;
                return *(sub*)this;
            }
        };
        ...
        template<class sub>
        std::map<std::string, sub*> recorder<sub>::list;
    }
    ...
    class quantity : public registry::recorder< quantity<T> > {
      public:
        ...
        quantity(const quantity & q,
                 const std::string & _name = "", const regname & rs = "") :
            registry(rs),
