    ...
    namespace map {
      ...
      */
      ...
      struct pivot_ctrs : DIMS {
        ...
        pivot_ctrs(const Vector<double,3> & pivot) {
          DIMS::pivot = pivot;
        }
      
        template <class Info>
        pivot_ctrs(const Info & info, const Vector<double,3> & pivot = 0.0) : DIMS(info) {
          DIMS::pivot = pivot;
        }
        /* ** END CONSTRUCTORS ** */
      };
