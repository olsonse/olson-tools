...
namespace olson_tools{
  namespace fit {
    ...
    inline Histogram::Histogram( int population,
                                 int cont_grid_cols,
                                 const Individual<MF> & member )
    ...
    template < typename MF >
    inline merit_t Histogram::meritfact(Individual<MF> & member) {
    ...
    template < typename MF >
    inline void Histogram::update(Individual<MF> & member) {
      ...
      for(int i=0; i<rows; ++i) {
        ...
        if ( j == ALLELE_DYNAMIC_CONT ) {
          j = int(
                   (   member.DNA.Allele( i, ALLELE_DYNAMIC ).max
                     - member.DNA.Allele( i, ALLELE_DYNAMIC ).val )
                   / grid_spacing
                 ) % colsize[i];
        } else {
    ...
    inline std::ostream & operator<<(std::ostream &output, const Histogram & hist) {
