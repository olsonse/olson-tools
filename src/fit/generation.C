// -*- c++ -*-
// $Id: generation.C,v 1.1 2005/01/08 04:27:25 olsonse Exp $
/*
 * $Log: generation.C,v $
...
#if DIVERSITY != DNONE
  Histogram hist(population, hist_grid_spacing, *member[0]);
#endif
  for(int i=0; i<population; i++){
#ifdef HYBRID
...
Generation::Generation(int pop, const Gene & igene,
  ...
  mutprob = 0.05;
  //Histogram defaults
  hist_grid_spacing = 1; //This will have to be dynamically changeable
...
Generation::Generation(const Generation& gen):
  ...
  hist_grid_spacing(gen.hist_grid_spacing) {
...
void Generation::tournament(){
  ...
  while( ichild < population && !stop ){
    ...
    int ip1 = int(range_rand(0,population-1));
    int ip2=ip1;
    while(ip2 == ip1){
      ip2 = int(range_rand(0,population-1));
    ...
    {// Now do it again for another parent
      ...
      while((ip3 == ip2) || (ip3 == ip1)){
        ip3 = int(range_rand(0,population-1));
      } // while parents are identical
      // compare parents and keep the best one
...
void Generation::seed(const Gene & seedgene, float seedprob){
  // insure inclusion of a specific solution in the set
  int numseed = int(seedprob*population);
  for(int i=0;i<numseed; i++) {
    float p = range_rand(0,1);
    if( p <= seedprob ) member[i]->regene(seedgene); 
  }//for
} // Generation::seed
...
Histogram::Histogram(int population, float grid_dx, const Individual & member):
  grid_spacing(grid_dx), mfact(1.0 - 1.0/sqrt(float(population))) {
  // allocate some space for the accumulation arrays
  if(!(rows = member.DNA.Numalleles(ALLELE_DYNAMIC)))
    throw libfitError(EMPTY_DYNAMIC_DNA);
...
merit_t Histogram::meritfact(Individual& member){
  ...
  for(int i=0; i<rows; i++) {
    ...
    if( j == ALLELE_DYNAMIC_CONT) {//if match, set j to second index
      j = int(member.DNA.Allele( i, ALLELE_DYNAMIC ).val
              / grid_spacing) % colsize[i];
    }
    else {
      j = int( member.DNA.Allele( i, ALLELE_DYNAMIC ).val ) % colsize[i];
    }
...
fit_t Gene_Simplex::minfunc(Allele_t p[]){
...
///
Individual ** new_Individual_list(const Gene & gn, int population,
                                  CREATE_IND_FUNC createind = NULL,
                                  void * meritfnc = NULL,
                                  void * obj_ptr = NULL ) {
...
void delete_Individual_list(Individual ** ilist) {
  --ilist;
  int population = (int)ilist[0];
  ilist++;
  for(int i = 0; i<population; delete ilist[i++]);
  delete --ilist;
} //delete_population_list

ostream & operator<<(ostream &output, const Generation & gen) {
...
ostream & operator<<(ostream &output, const Histogram & hist) {
