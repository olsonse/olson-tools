// -*- c++ -*-
// $Id: generation.h,v 1.1 2005/01/08 04:27:25 olsonse Exp $
/*
 * $Log: generation.h,v $
...
// If DIVERSITY is true, the program tries to improve diversity by penalizing
// too many individuals of the same species in the population.
// If it is equal to DMAT, it penalizes all stacks made of the same materials
// If it is equal to DALL, it penalize them if their materials match and their
// thicknesses are within DPERCENT of each other.

#define DNONE 0
#define DIVERSITY DNONE
...
public:
  ...
  */
  ...
  static bool stop;
  ///
  friend ostream & operator<<(ostream &,const Generation &);
private:
  ...
  /// This is the grid spacing of coordinate-system type genes in histogramming
  float hist_grid_spacing;

  //int histboolean; // if true then do the Histogramming
  //Histogram hist; // the histogram object
}; // Generation class
...
public:
  ...
  */
  ...
  ///
  virtual fit_t minfunc(Allele_t p[]);
private:
  ///
  Individual * member;
}; // Gene_Simplex class
...
public:
  ///
  Histogram(int population, float grid_dx, const Individual &);
  ///
  ~Histogram();
...
private:
  ...
  float grid_spacing;///refers to grid spacing of histogram for coordinate system gene
  int rows;
  ///
  int *colsize;
  ///
  merit_t mfact; //factor to coerce smaller populations to have larger diversity
}; // Histogram

///The Generation print function.
ostream & operator<<(ostream &, const Generation &);

///The Histogram print function.
ostream & operator<<(ostream &, const Histogram &);
